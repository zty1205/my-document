    ...
    <div class="body">
      <div class="block">
        ...
        <div class="inline-block-demo">
          ...
          <el-button type="primary" @click="toggle('currentComp')">切换</el-button>
        </div>
        ...
        <div class="inline-block-demo">
          <p>使用v-if命令</p>
            ...
            <el-button type="primary" @click="toggle('currentIf')">切换</el-button>
        </div>
      </div>
      ...
      <div class="block">
        <div class="inline-block-demo">
          <p>使用include命令</p>
            ...
            <el-button v-for="item in compList" :key="item" type="primary" 
              @click="currentCompInclude = `comp${item}`">切换{{ item }}</el-button>
        </div>
        ...
        <div class="inline-block-demo">
          <p>使用exclude命令</p>
            ...
            <el-button v-for="item in compList" :key="item" type="primary" 
              @click="currentCompExclude = `comp${item}`">切换{{ item }}</el-button>
        </div>
      </div>
      ...
      <div class="block">
        <div class="inline-block-demo">
          ...
          <el-button v-for="item in compList" :key="item" type="primary" 
            @click="currentCompMax = `comp${item}`">切换{{ item }}</el-button>
        </div>
      </div>
    </div>
  </div>
...
for(let i = 0, len = compList.length; i < len; i++) {
  ...
  Vue.component(`comp${tag}`, {
    render: function(h) { // 不应该使用箭头函数 否则会找不到this
