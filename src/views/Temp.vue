  ...
  methods: {
    ...
    calcLineWidth(t, s){
      ...
      if(v <= MIN_STROKE_Y) {
        ...
        resultLineWidth = MAX_LINE_WIDTH - (v-MIN_STROKE_Y)/(MAX_STROKE_Y-MIN_STROKE_Y)*(MAX_LINE_WIDTH-MIN_LINE_WIDTH);
      }
      if (this.lastLineWidth === -1 ) {
        return resultLineWidth;
      }
      // 使线条更平滑
      return resultLineWidth * (1 - LAST_LINE_RADIO) + this.lastLineWidth * LAST_LINE_RADIO;
    },
    lineBegin(point) {
    ...
    lineMove(point) {
      ...
      let lineWidth = this.calcLineWidth(curTimeStamp - this.lastTimeStamp, dis)
